...
To use an OCI image registry other than a local registry in the cluster (with a registry address `localhost:5001`), you will need to modify the repository in the `build.gradle.kts` files for the link:../../books-server/build.gradle.kts[`books-server`] and link:../../books-client/build.gradle.kts[`books-client`] subprojects.
